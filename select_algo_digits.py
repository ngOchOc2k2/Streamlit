...
def Select_Algo(algo):
    if algo == 'Neural Network':
        ...
        if pre:
            labels = model_digits.KNN(X_train, y_train, X_test, y_test, neighbors=neighbors, p=norms)
            st.write('Accuracy is ', 100 * accuracy_score(labels, y_test))

    elif algo == 'KMeans':
        model_digits.K_means(X_train, y_train, X_test, y_test, n_cluster=10)

    elif algo == 'Softmax Regression':
        labels_sm, huhi, abc = model_digits.Softmax(X_test=X_test)
        ...
        if if_draw:
            ...
            with col[0]:
                ...
                if canvas_result.image_data is not None:
                    ...
                    x_ori = x_img
                    st.write(X_trains[0].shape)
                    st.write(x_img.shape)
                    x_img = x_img.reshape(-1, 784)/255.
                    labels_sms, huhis, abcs = model_digits.Softmax(X_test=x_img)
        ...
        if if_maxtrixdigit:
            st.write(x_ori)
            st.write(x_ori.shape)

    elif algo == 'SVM':
        a, b = model_digits.SVM(X_train=X_train, y_train=y_train)
        st.write(a)
        st.write(b)
    elif algo == 'MLP':
        # Define parameters
        ...
        coltrain = st.columns(3)
        with coltrain[0]:
            WeightData = st.selectbox('Show weights', options=('None', 'Show weight'))
        with coltrain[1]:
            PlotData = st.selectbox(label='Plot', options=('Accuracy', 'Loss'))
        with coltrain[2]:
            DrawData = st.selectbox('Draw Data', options=('None', 'Draw'))
        ...
        if DrawData == 'Draw':
            load_model = tf.keras.models.load_model("D:\MSAVE")
            col = st.columns(2)
            with col[0]:
                drawing_mode = 'freedraw'
                st.write('### Draw pixel 28x28')
                ...
                if canvas_result.image_data is not None:
                    ...
                    # x_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                    # x_ori = x_img
                    # x_img_mlp = x_img.reshape(-1, 784)
                    label_image = model_digits.MLP(image1, load_model).flatten()
                    # st.write(label_image)
                    with col[1]:
                        st.bar_chart(label_image)
